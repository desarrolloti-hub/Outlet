...
async function loadCustomerData() {
    try {
        ...
        // 🔧 CORRECCIÓN: Verificar si la foto está en customer y actualizar sesión
        var fotoUrl = customer.fotoPerfil || customer.photoURL || null;
        if (fotoUrl && fotoUrl.startsWith('http')) {
            var session = JSON.parse(localStorage.getItem('outlet_customer') || '{}');
            session.fotoPerfil = fotoUrl;
            session.photoURL = fotoUrl;
...
function loadDataToForm(customer) {
    ...
    if (nombre) nombre.value = customer.nombre || customer.displayName || '';
    ...
    if (destinatario) destinatario.value = direccion.destinatario || customer.nombreCompleto || customer.displayName || '';
...
// ========================================
// Actualizar avatar del editUser
// ========================================
function updateEditUserAvatar() {
    try {
        ...
        if (nameDisplay) {
            nameDisplay.textContent = session.nombreCompleto || session.nombre || session.displayName || 'Usuario';
        }
        if (emailDisplay) {
        ...
        // Calcular iniciales
        var nombre = session.nombre || session.displayName || '';
        ...
        // 🔧 Buscar la foto en múltiples propiedades
        var fotoUrl = session.fotoPerfil ||
            session.photoURL ||
            session.foto ||
            session.avatar ||
            session.picture ||
            (session.providerData && session.providerData[0] && session.providerData[0].photoURL);
...
function actualizarIniciales() {
    ...
    var nombre = nombreInput?.value.trim() || currentCustomer?.nombre || currentCustomer?.displayName || '';
        ...
        avatarInput.addEventListener('change', async function (e) {
            ...
            try {
                ...
                if (result.success && result.fotoUrl) {
                    ...
                    session.photoURL = result.fotoUrl;
                    localStorage.setItem('outlet_customer', JSON.stringify(session));
