...
async function handleGoogleLogin() {
    ...
    try {
        ...
        if (result && (result.adminData?.id || result.customerData?.id)) {
            const userId = result.adminData?.id || result.customerData?.id;
            const fcmRole = result.adminData ? 'admin' : 'customer';
            // Ejecutar en background sin bloquear
            AuthService.saveFcmTokenAfterLogin(userId, fcmRole).catch(err => {
                console.error('Error guardando token FCM (no crítico):', err);
            });
        }
        // =========================================================
