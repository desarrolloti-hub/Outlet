...
export const AuthService = {
    ...
    /**
     * Guarda el token FCM después de login/registro exitoso
     * Se ejecuta en background sin bloquear la UI
     *
     * @param {string} userId
     * @param {'customer'|'admin'} [role] - si no se indica, se infiere con getUserRoleSync()
     */
    async saveFcmTokenAfterLogin(userId, role = null) {
        try {
            if (!userId) {
                console.warn('⚠️ No hay userId para guardar token');
                return null;
            }
            ...
            // Solo si el usuario ya concedió permisos
            if (Notification.permission !== 'granted') {
                console.log('ℹ️ El usuario debe activar notificaciones manualmente');
                return null;
            }

            const resolvedRole = role || this.getUserRoleSync();

            // Obtener token FCM y guardarlo (initPush ya lo persiste en
            // clientes/administradores según el rol indicado)
            const { NotificationService } = await import('./notificationService.js');
            const token = await NotificationService.initPush(userId, resolvedRole);
